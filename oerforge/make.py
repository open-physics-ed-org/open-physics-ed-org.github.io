...
def generate_nav_menu(context: dict) -> str:
    ...
    # Build navigation menu from the database
    db_path = os.path.join(PROJECT_ROOT, 'db', 'sqlite.db')
    conn = sqlite3.connect(db_path)
    cursor = conn.cursor()
    # Only include menu entries (is_autobuilt or markdown-backed, and menu flag true)
    cursor.execute("SELECT title, output_path, slug, source_path FROM content WHERE (parent_output_path IS NULL OR parent_output_path = '') AND (source_path IS NOT NULL OR is_autobuilt = 1)")
    nav_html = '<nav class="site-nav" role="navigation" aria-label="Main menu"><ul>'
    for title, output_path, slug, source_path in cursor.fetchall():
        # If markdown-backed, use output_path; if autogenerated, use slug/index.html
        if source_path:
            link = output_path
        else:
            link = slug + '/index.html' if slug else output_path
...
def build_all_markdown_files():
    ...
    for source_path, output_path, db_title, mime_type in records:
        ...
        if ext == '.md' or (mime_type and 'markdown' in mime_type.lower()):
            ...
            nav_html = generate_nav_menu({'toc': toc})
            rel_path = os.path.relpath(output_path, BUILD_HTML_DIR)
